...
async function requestOpenAiAnalysis(payload) {
  if (!OPENAI_API_KEY) {
    return {
      model: null,
      message:
        "OPENAI_API_KEYê°€ ?¤ì •?˜ì? ?Šì•„ ë¡œì»¬ ë¶„ì„ ê²°ê³¼ë§??€?¥í–ˆ?µë‹ˆ?? ?˜ê²½ë³€?˜ë? ?¤ì •?˜ë©´ GPT ?”ì•½ê³?ìµœì ???œì•ˆ???¨ê»˜ ?ì„±?©ë‹ˆ??",
    };
  }

  const prompt = [
    "?¹ì‹ ?€ ë²„ìŠ¤/?•ë¥˜???¸ì„  ?°ì´??ë¶„ì„ê°€?…ë‹ˆ??",
    "?…ë ¥ ?°ì´?°ì—???¸ì„ ë³??¬ì¸?¸ì? ê²½ë¡œ ì¢Œí‘œ, ê·¸ë¦¬ê³?ë¡œì»¬ ë¶„ì„ ê²°ê³¼ê°€ ?¬í•¨?©ë‹ˆ??",
    "?´ì•¼ ????",
    "1. ë°˜ê²½ 30m ??ì¤‘ë³µ ?•ë¥˜???„ë³´ë¥??´ì„?œë‹¤.",
    "2. ?¸ì„  ê°?ê²½ë¡œ ì¤‘ë³µ êµ¬ê°„ ?„ë³´ë¥??´ì„?œë‹¤.",
    "3. ?´ì˜???˜ë? ?ˆëŠ” ì¤‘ë³µê³??¨ìˆœ ê·¼ì ‘??êµ¬ë¶„?œë‹¤.",
    "4. ìµœì ???°ì„ ?œìœ„ë¥??œì•ˆ?œë‹¤.",
    "ì¶œë ¥ ?•ì‹:",
    "- summary: 4ë¬¸ìž¥ ?´ë‚´ ?”ì•½",
    "- duplicate_stop_insights: ë¬¸ìž??ë°°ì—´",
    "- overlapping_path_insights: ë¬¸ìž??ë°°ì—´",
    "- optimization_actions: ë¬¸ìž??ë°°ì—´",
    "- risks: ë¬¸ìž??ë°°ì—´",
    "",
      ...
      readRequestBody(req)
        .then(async (rawBody) => {
          if (!KAKAO_MOBILITY_REST_API_KEY) {
            sendJson(res, 500, { error: "KAKAO_MOBILITY_REST_API_KEY ?˜ê²½ë³€?˜ê? ?¤ì •?˜ì? ?Šì•˜?µë‹ˆ??" });
            return;
          }
          ...
          const routeName = String(payload.routeName || "?¤ê³„ ?¸ì„ ");

          if (points.length < 2) {
            sendJson(res, 400, { error: "?¸ì„  ?¤ê³„ë¥??˜ë ¤ë©??¬ì¸?¸ê? 2ê°??´ìƒ ?„ìš”?©ë‹ˆ??" });
            return;
          }
          ...
          const kakaoResponse = await fetch("https://apis-navi.kakaomobility.com/v1/waypoints/directions", {
            ...
            body: JSON.stringify({
              origin: {
                x: Number(origin.lng),
                y: Number(origin.lat),
                name: String(origin.name || "ì¶œë°œì§€"),
              },
              destination: {
                x: Number(destination.lng),
                y: Number(destination.lat),
                name: String(destination.name || "?„ì°©ì§€"),
              },
              waypoints,
              summary: false,
              ...designOptions,
          ...
          if (!kakaoResponse.ok) {
            ...
            sendJson(res, kakaoResponse.status, {
              error: `ì¹´ì¹´??ëª¨ë¹Œë¦¬í‹° ?¸ì„  ?¤ê³„ ?”ì²­ ?¤íŒ¨: ${errorText}`,
            });
            return;
          }
